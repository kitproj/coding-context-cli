	...
	tests := []struct {
		...
		},
		{
			...
			check: func(t *testing.T, task Task) {
				...
				if task[0].ShellCommand == nil {
					t.Fatal("expected shell command block")
				}
				if task[0].ShellCommand.Command() != "echo hello" {
					t.Errorf("expected command 'echo hello', got %q", task[0].ShellCommand.Command())
				}
			},
		},
		{
			name:    "shell command with complex command",
			...
			check: func(t *testing.T, task Task) {
				...
				if task[0].ShellCommand.Command() != "git log --oneline -10" {
					t.Errorf("expected command 'git log --oneline -10', got %q", task[0].ShellCommand.Command())
				}
			},
		},
		{
			name:    "shell command with text before and after",
			input:   "Recent commits:\n!`git log --oneline -5`\nReview these changes.",
			wantErr: false,
			check: func(t *testing.T, task Task) {
				if len(task) != 3 {
					t.Fatalf("expected 3 blocks, got %d", len(task))
				}
				if task[0].Text == nil {
					t.Fatal("expected block 0 to be text")
				}
				if !strings.Contains(task[0].Text.Content(), "Recent commits:") {
					t.Errorf("expected text to contain 'Recent commits:', got %q", task[0].Text.Content())
				}
				if task[1].ShellCommand == nil {
					t.Fatal("expected block 1 to be shell command")
				}
				if task[1].ShellCommand.Command() != "git log --oneline -5" {
					t.Errorf("expected command 'git log --oneline -5', got %q", task[1].ShellCommand.Command())
				}
				if task[2].Text == nil {
					t.Fatal("expected block 2 to be text")
				}
				if !strings.Contains(task[2].Text.Content(), "Review these changes.") {
					t.Errorf("expected text to contain 'Review these changes.', got %q", task[2].Text.Content())
				}
			},
		},
		{
			...
			check: func(t *testing.T, task Task) {
				if len(task) != 3 {
					t.Fatalf("expected 3 blocks, got %d (text between commands)", len(task))
				}
				if task[0].ShellCommand == nil {
					t.Fatal("expected block 0 to be shell command")
				}
				if task[0].ShellCommand.Command() != "echo first" {
					t.Errorf("expected command 'echo first', got %q", task[0].ShellCommand.Command())
				}
				if task[2].ShellCommand == nil {
					t.Fatal("expected block 2 to be shell command")
				}
				if task[2].ShellCommand.Command() != "echo second" {
					t.Errorf("expected command 'echo second', got %q", task[2].ShellCommand.Command())
				}
			},
		},
	}
