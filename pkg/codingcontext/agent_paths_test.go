...
func TestAgentPaths_Structure(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !exists {
				t.Errorf("AgentsPaths[%q] does not exist", tt.agent)
				return
			}
			...
			if !hasAnyPath {
				t.Errorf("AgentsPaths[%q] has no paths defined", tt.agent)
			}
		})
	}
}
...
func TestAgentPaths_EmptyAgentHasAllPaths(t *testing.T) {
	...
	if !exists {
		t.Fatal("Empty agent should be defined in AgentsPaths")
	}
